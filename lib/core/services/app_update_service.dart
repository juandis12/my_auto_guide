...
class AppUpdateService {
  ...
  Future<String?> downloadAndExtractApk({
    ...
    try {
      ...
      final zipFilePath = p.join(tempDir.path, 'update_release.zip');
      ...
      await for (final chunk in response.stream) {
        ...
        if (contentLength > 0) {
          // Asignar el 0% a 75% del progreso a la descarga
          final downloadRatio = (downloadedBytes / contentLength).clamp(0.0, 1.0);
          final progress = 0.05 + (downloadRatio * 0.70);
          ...
          onProgress(0.40, 'Descargando actualización...');
        }
      }
      ...
      if (extractedApkPath == null) {
        throw Exception('El archivo ZIP no contiene ningún archivo .apk válido.');
      }

      onProgress(1.0, '¡Descarga y descompresión completadas!');
